...
type NodeChangeNotifyFunc func(zkwatch *CZkWatch, asynConn *AsyncConn, path string, evtype EventType)

type CZkLocalTree struct {
	mtree map[string][]string
}

func (t *CZkLocalTree) nodeContains(node string, tarNodes []string) bool {
...
func (t *CZkLocalTree) nodeChildChange(path string, newChilds []string) ([]string, []string, []string) {
	var addNodes []string
	var delNodes []string
	var curChilds []string
	for _, v := range newChilds {
		curChilds = append(curChilds, v)
	}

	_, ok := t.mtree[path]
	if !ok {
		t.mtree[path] = curChilds
		addNodes = curChilds
	} else {
		existChilds := t.mtree[path]
		for _, v := range curChilds {
			if ok := t.nodeContains(v, existChilds); !ok {
				addNodes = append(addNodes, v)
			}
		}

		for _, v := range existChilds {
			if ok := t.nodeContains(v, curChilds); !ok {
	...
	return addNodes, delNodes, curChilds
}

type CZkWatch struct {
	watchfunc    map[string]NodeChangeNotifyFunc
	watchersLock sync.Mutex
	localTree    *CZkLocalTree
}

func NewCZkWatch() *CZkWatch {
	c := new(CZkWatch)
	c.watchfunc = make(map[string]NodeChangeNotifyFunc)
	c.localTree = new(CZkLocalTree)
	c.localTree.mtree = make(map[string][]string)
	return c
}

func (c *CZkWatch) AddNodeChangeNotifyFunc(path string, notifyfunc NodeChangeNotifyFunc) {
	c.watchfunc[path] = notifyfunc
}

func (c *CZkWatch) NodeEvent(asynConn *AsyncConn, path string, evtype EventType) {
	prefix := ZookeeperPathGetPrefix(path)
	if _, ok := c.watchfunc[prefix]; ok {
		c.watchfunc[prefix](c, asynConn, path, evtype)
	}
}

func (c *CZkWatch) DispatchToSubNodeChange(asynConn *AsyncConn, path string) (bool, []string, error) {
	subNames, _, err := asynConn.Children(path, true)
	if nil != err {
		return false, nil, err
	}

	addNodes, delNodes, curChilds := c.localTree.nodeChildChange(path, subNames)

	for _, v := range addNodes {
		c.NodeEvent(asynConn, path+"/"+v, EventNodeCreated)
	}

	for _, v := range delNodes {
		c.NodeEvent(asynConn, path+"/"+v, EventNodeDeleted)
	}

	return true, curChilds, nil
}

func (c *CZkWatch) Init(asynConn *AsyncConn, ec chan Event) error {
	go func() {
		c.loopNotify(asynConn, ec)
	}()

	return nil
}

func (c *CZkWatch) loopNotify(asynConn *AsyncConn, ec chan Event) error {
	for {
		select {
		case ev := <-ec:
			c.watchersLock.Lock()
			prefix := ZookeeperPathGetPrefix(ev.Path)
			if _, ok := c.watchfunc[prefix]; ok {
				c.watchfunc[prefix](c, asynConn, ev.Path, ev.Type)
			}
...
type connOption func(c *AsyncConn)
...
func ConnectWithDialer(servers []string, sessionTimeout time.Duration, zkwatch *CZkWatch, dialer Dialer) (*AsyncConn, error) {
	return Connect(servers, sessionTimeout, zkwatch, WithDialer(dialer))
}
...
func Connect(servers []string, sessionTimeout time.Duration, zkwatch *CZkWatch, options ...connOption) (*AsyncConn, error) {
	if len(servers) == 0 {
	...
	zkwatch.Init(conn, ec) //initialize zkwatch and start to notify
...
func (c *AsyncConn) SetLogger(l Logger) {
	c.logger = l
}

func (c *AsyncConn) setTimeouts(sessionTimeoutMs int32) {
...
func (c *AsyncConn) setState(state State) {
	atomic.StoreInt32((*int32)(&c.state), int32(state))
	c.sendEvent(Event{Type: EventSession, State: state, Server: c.Server()})
}

func (c *AsyncConn) sendEvent(evt Event) {
...
func (c *AsyncConn) connect() error {
	var retryStart bool
	for {
		c.serverMu.Lock()
...
func (c *AsyncConn) resendZkAuth(reauthReadyChan chan struct{}) {
...
func (c *AsyncConn) flushUnsentRequests(err error) {
	for {
		select {
		default:
			return
...
func (c *AsyncConn) flushRequests(err error) {
	c.requestsLock.Lock()
...
/*func (c *AsyncConn) invalidateWatches(err error) {
	c.watchersLock.Lock()
...
func (c *AsyncConn) sendSetWatches() {
	c.watchersLock.Lock()
	defer c.watchersLock.Unlock()
...
func (c *AsyncConn) authenticate() error {
	buf := make([]byte, 256)
...
func (c *AsyncConn) sendData(req *request) error {
...
func (c *AsyncConn) sendLoop() error {
...
func (c *AsyncConn) recvLoop(conn net.Conn) error {
	sz := bufferSize
...
func (c *AsyncConn) nextXid() int32 {
	return int32(atomic.AddUint32(&c.xid, 1) & 0x7fffffff)
}

/*func (c *AsyncConn) addWatcher(path string, watchType watchType) <-chan Event {
	c.watchersLock.Lock()
	defer c.watchersLock.Unlock()
...
func (c *AsyncConn) queueRequest(opcode int32, req interface{}, res interface{}, recvFunc func(*request, *responseHeader, error)) <-chan response {
	rq := &request{
		xid:        c.nextXid(),
		opcode:     opcode,
		pkt:        req,
...
func (c *AsyncConn) request(opcode int32, req interface{}, res interface{}, recvFunc func(*request, *responseHeader, error)) (int64, error) {
	r := <-c.queueRequest(opcode, req, res, recvFunc)
	return r.zxid, r.err
}

func (c *AsyncConn) AddAuth(scheme string, auth []byte) error {
...
func (c *AsyncConn) Children(path string, isWatch bool) ([]string, *Stat, error) {
	if err := validatePath(path, false); err != nil {
		return nil, nil, err
	}

	res := &getChildren2Response{}
	_, err := c.request(opGetChildren2, &getChildren2Request{Path: path, Watch: isWatch}, res, nil)
	return res.Children, &res.Stat, err
}

/*func (c *AsyncConn) ChildrenW(path string) ([]string, *Stat, <-chan Event, error) {
	if err := validatePath(path, false); err != nil {
...
func (c *AsyncConn) Get(path string, isWatch bool) ([]byte, *Stat, error) {
	if err := validatePath(path, false); err != nil {
		return nil, nil, err
	}

	res := &getDataResponse{}
	_, err := c.request(opGetData, &getDataRequest{Path: path, Watch: isWatch}, res, nil)
	return res.Data, &res.Stat, err
}

// GetW returns the contents of a znode and sets a watch
/*func (c *AsyncConn) GetW(path string) ([]byte, *Stat, <-chan Event, error) {
	if err := validatePath(path, false); err != nil {
		return nil, nil, nil, err
	}
...
func (c *AsyncConn) Set(path string, data []byte, version int32) (*Stat, error) {
	if err := validatePath(path, false); err != nil {
		return nil, err
	}
...
func (c *AsyncConn) Create(path string, data []byte, flags int32, acl []ACL) (string, error) {
...
func (c *AsyncConn) CreateProtectedEphemeralSequential(path string, data []byte, acl []ACL) (string, error) {
	...
	for i := 0; i < 3; i++ {
		...
		case ErrConnectionClosed:
			children, _, err := c.Children(rootPath, false)
			if err != nil {
				return "", err
			}
...
func (c *AsyncConn) Delete(path string, version int32) error {
	if err := validatePath(path, false); err != nil {
		return err
	}
...
func (c *AsyncConn) Exists(path string, isWatch bool) (bool, *Stat, error) {
	if err := validatePath(path, false); err != nil {
		return false, nil, err
	}

	res := &existsResponse{}
	_, err := c.request(opExists, &existsRequest{Path: path, Watch: isWatch}, res, nil)
	exists := true
	if err == ErrNoNode {
		exists = false
		err = nil
...
/*func (c *AsyncConn) ExistsW(path string) (bool, *Stat, <-chan Event, error) {
	if err := validatePath(path, false); err != nil {
...
func (c *AsyncConn) GetACL(path string) ([]ACL, *Stat, error) {
	if err := validatePath(path, false); err != nil {
		return nil, nil, err
	}
...
func (c *AsyncConn) SetACL(path string, acl []ACL, version int32) (*Stat, error) {
	if err := validatePath(path, false); err != nil {
		return nil, err
	}
...
func (c *AsyncConn) Sync(path string) (string, error) {
	if err := validatePath(path, false); err != nil {
		return "", err
	}
...
// *CheckVersionRequest.
func (c *AsyncConn) Multi(ops ...interface{}) ([]MultiResponse, error) {
	req := &multiRequest{
...
func (c *AsyncConn) IncrementalReconfig(joining, leaving []string, version int64) (*Stat, error) {
...
func (c *AsyncConn) Reconfig(members []string, version int64) (*Stat, error) {
	request := &reconfigRequest{
...
func (c *AsyncConn) internalReconfig(request *reconfigRequest) (*Stat, error) {
...
func (c *AsyncConn) Server() string {
	c.serverMu.Lock()
